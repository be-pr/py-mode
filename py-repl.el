  ...
  (let* ((buf (py-repl-process-buffer))
    ...
    (save-excursion
      ...
      (unless (py-indent--beginning-of-block-p)
        (error "No block statement at point"))
      (let ((beg (point)))
...
(defun py-eval-simple-statement ()
  (interactive)
  (let* ((buf (py-repl-process-buffer))
         (proc (get-buffer-process buf))
         (orig (point)))
    ...
    (save-excursion
      ...
      (let ((state (syntax-ppss))
        ...
        (unless (zerop (car state))
          (goto-char (car (nth 9 state)))
          (setq end (scan-lists (point) 1 0)))
        (forward-line 0)
        (forward-comment (buffer-size))
        ;; Empty or comment line.
        (when (> (point) orig) (error "No statement at point"))
        (process-send-string proc "eval(compile(r'''")
        (process-send-region proc (point) end)
        (process-send-string proc "\n''', '<stdin>', 'single'))\n")))))
